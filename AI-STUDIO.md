...
EXCEÇÃO DOCUMENTADA (analytics): o projeto proíbe JS inline, mas os snippets de analytics no `<head>` (todas as 5 páginas) são a exceção padrão para código base de analytics: Meta Pixel (id 999436976070756) e Google Analytics 4 / gtag.js (Measurement ID G-ZQB2W7YE9M). NÃO remover achando que é violação da regra de inline JS.

CONTRATO FINAL DO META PIXEL (não alterar sem alinhamento com o projeto de Meta Ads): as LPs NÃO carregam o `fbevents.js`. O PageView é enviado por image pixel direto (`new Image()` para `https://www.facebook.com/tr?id=...&ev=PageView&dl=...`), dentro da trava de domínio de produção (`vinicavalcanti.com` / `*.vinicavalcanti.com`), mais o fallback `<noscript>` (que já é image pixel). MOTIVO: o motor de inferência interno do fbevents.js gerava InitiateCheckout FALSO no clique dos CTAs (Initiator: fbevents.js), imune ao `autoConfig=false` local (a config remota do Pixel sobrepõe) e a wrappers no `window.fbq` (a inferência não passa pela API pública). Todo evento de funil (InitiateCheckout, Purchase) nasce exclusivamente na integração nativa da Hotmart em pay.hotmart.com. NÃO reintroduzir fbevents.js/fbq nas páginas.
